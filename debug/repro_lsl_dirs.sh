...
mkdir -p "$LOG_DIR"
TS="$(date +%Y%m%d-%H%M%S)"
UART_LOG="$LOG_DIR/repro-lsl-dirs-uart-$TS.log"
UART_RAW="$LOG_DIR/repro-lsl-dirs-uart-raw-$TS.log"
...
run_lsl_dir() {
  ...
  local start="__LSL_BEGIN_${safe}__"
  local end="__LSL_END_${safe}__"

  echo "Running: ls -l $dir"
  kor_retry uart-inject "echo ${start}\\nls -l ${dir}\\necho ${end}\\n" >/dev/null
  if ! wait_for_token "$end" "lsl_${safe}"; then
    echo "Timeout waiting for $end"
    sample_state "lsl_${safe}/timeout"
    return 1
  fi
  return 0
}
...
# Keep initrd last, as requested.
DIRS=(/ bin dev etc init lib media mnt proc root run sbin sys tmp usr var initrd)
